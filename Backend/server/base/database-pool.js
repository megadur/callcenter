...
function terminatePool(sAlias) {
    return new Promise(function (resolve, reject) {
        let pool = getPool(sAlias);
        if (pool) {
...
function getPool(sAlias) {
    pools.forEach(function (item, index, array) {
        if (item.poolAlias == sAlias) {
            // console.log('getPool: ', sAlias, ' found ', item.poolAlias, index);
            return item;
        }
    });
    return pool;
